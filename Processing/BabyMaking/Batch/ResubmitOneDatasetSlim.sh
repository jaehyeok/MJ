...
while [ $COUNTER -lt $(($LASTFILENUM)) ]; do
    ...
    then 
        ...
        then
            echo "JobSubmit.csh ./RunOneJob.sh  $INPUTDIR/$DATASET/slim/ $RECOGNIZER $COUNTER $(($LASTFILENUM)) $ISDATA $LUMI"
        ...
        if [ ! -e /net/cms26/cms26r0/jaehyeok/baby/baby_${RECOGNIZER}_f${BEGINFILE}To${ENDFILE}.root ] 
        then
            echo "JobSubmit.csh ./RunOneJob.sh $INPUTDIR/$DATASET/slim/ $RECOGNIZER $COUNTER $(($COUNTER+$NFILEPERJOB-1)) $ISDATA $LUMI" 
