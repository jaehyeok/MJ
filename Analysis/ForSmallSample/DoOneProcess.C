...
// main function
//
void DoOneProcess(TString InputCfAName, TString ProcessName, bool isData, float Lumi) 
{
    //
    cout << "[MJ Analysis] ------------------------------------------------------------------------------------"<<endl; 
    cout << "[MJ Analysis] Processing " << ProcessName << endl;

    TFile *babyFile_ = new TFile(Form("baby/baby_%s.root", ProcessName.Data()), "RECREATE");
    babyFile_->cd();
