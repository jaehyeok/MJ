...
void EventDisplayForBaby(bool sig=false, bool truth=true, bool fatjet=true, int event=-1)
{
    ...
    //if(sig) ch->Add("babies/small_quick_cfA_751_nleps1_trig0ON.root");
    //if(sig) ch->Add("small_quick_HTMHT_Run2015C-PromptReco-v1_MINIAOD_v83.root");
    if(sig) ch->Add("2015_11_05/data/singlelep/combined/skim_1lht500met200/*root");
    ...
    for(int i = 0; i<nentries; i++)
    {
        ...
        if(event>0 && event_!=event) continue; // FIXME   
        ...
        if(nels_==1)  
        {
            for(int i=0; i<RA4ElsPt_->size(); i++) 
            {
                if(RA4ElsMinIso_->at(i)>0.1) continue;
                if(IsRA4Els_->at(i)!=1) continue;
                if(RA4ElsPt_->at(i)<20) continue;
                lepPt   =RA4ElsPt_->at(i);
                lepEta  =RA4ElsEta_->at(i);
                lepPhi  =RA4ElsPhi_->at(i); 
            }
        }
        if(nmus_==1)  
        {
            for(int i=0; i<RA4MusPt_->size(); i++) 
            {
                if(RA4MusMinIso_->at(i)>0.2) continue;
                if(IsRA4Mus_->at(i)!=1) continue;
                if(RA4MusPt_->at(i)<20) continue;
                lepPt   =RA4MusPt_->at(i);
                lepEta  =RA4MusEta_->at(i);
                lepPhi  =RA4MusPhi_->at(i); 
            }
        }
        ...
        myText(xalign+0.01,0.76,Form("reco %s p_{T} = %.0f GeV", nvmus_==1?"#mu":"e", lepPt), kBlack, 0.04); 
        nextline=0.76;
        ...
        for(int ifj = 0; ifj< (int)FatjetPt_->size(); ifj++)
        {
            myText(xalign+0.01,nextline,Form("%3.0f, %2.1f, %2.1f, %3.0f",
                   FatjetPt_->at(ifj),FatjetEta_->at(ifj),FatjetPhi_->at(ifj),mj_->at(ifj)),1,0.03); 
        ...
        {            
            if(JetIsLep_->at(ij)==1) continue;
            myText(xalign+0.01,nextline,Form("%3.0f, %2.1f, %2.1f",
                   JetPt_->at(ij),JetEta_->at(ij),JetPhi_->at(ij)),1,0.03); 
            nextline=nextline-0.035;
        }
        ...
        for(int ij=0;ij<(int)JetPt_->size();ij++)
        {
            ...
            cout << JetPt_->at(ij) << " " << JetEta_->at(ij) << " " <<  JetPhi_->at(ij) << endl;
                ...
                if(JetCSV_->at(ij)>0.890) jet.SetMarkerColor(8);
...
*/
        ...
        TString savename;
        if(!sig) savename = "fig/EventDisplay_"+cname+Form("%i_FJ",Nfatjet_thres); 
        else  savename = "fig/EventDisplay_"+cname+Form("%i_FJ",Nfatjet_thres);
...
/*
    event = 137500577; 
    EventDisplayForBaby(false, false, true, event);
